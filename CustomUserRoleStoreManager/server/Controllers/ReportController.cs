...
namespace CustomUserRoleStoreManager.Controllers
{
        ...
        static async void WriteResponse(HttpRequest currentReqest, string url, HttpResponseMessage responseMessage, HttpResponse response, bool isAjax)
        {
            ...
            if (isAjax && result.IndexOf("|") != -1)
            {
                ...
                while (index < result.Length) {
                    ...
                    content = content.Replace("/ReportServer/", String.Format("{0}/ReportServer/", proxyUrl));
                    content = content.Replace("./ReportViewer.aspx", String.Format("{0}/ReportServer/Pages/ReportViewer.aspx", proxyUrl));
