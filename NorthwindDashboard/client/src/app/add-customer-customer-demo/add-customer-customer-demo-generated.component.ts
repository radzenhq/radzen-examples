/*
  ...
  Modify add-customer-customer-demo.component.ts instead.
*/
...
export class AddCustomerCustomerDemoGenerated implements AfterViewInit, OnInit, OnDestroy {
  // Components
  ...
  load() {
    ...
    this.northwind.getCustomerDemographics(null, (<any>this.form0.fields.find(f => f.property == 'CustomerTypeID')).pageSize, 0, null, null, true, null, null)
    .subscribe((result: any) => {
  ...
  form0Submit(event: any) {
    this.northwind.createCustomerCustomerDemo(event)
    .subscribe((result: any) => {
      if (this.dialogRef) {
        this.dialogRef.close();
      }
      this.router.navigate(['customer-customer-demos']);
    }, (result: any) => {
      this.notificationService.notify({ severity: "error", summary: `Error`, detail: `Unable to create new CustomerCustomerDemo!` });
    });
  }
  ...
  form0LoadData(event: any) {
    ...
    if (event.property == 'CustomerTypeID') {
          this.northwind.getCustomerDemographics(`${event.filter}`, event.top, event.skip, `${event.orderby}`, null, true, null, null)
      .subscribe((result: any) => {
          this.getCustomerDemographicsResult = result.value;
