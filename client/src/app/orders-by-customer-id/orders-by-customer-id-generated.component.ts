...
export class OrdersByCustomerIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindOrders(`CustomerID eq '${this.parameters.CustomerID}'`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `Customer,Employee`, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getNorthwindOrders(`${event.filter ? event.filter + ' and ' : ''}CustomerID eq '${this.parameters.CustomerID}'`, event.top, event.skip, `${event.orderby}`, `Customer,Employee`, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getNorthwindOrdersResult = result.value;
