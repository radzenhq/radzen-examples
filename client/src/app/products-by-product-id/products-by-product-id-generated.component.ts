...
export class ProductsByProductIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindProducts(`ProductID eq ${this.parameters.ProductID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `Supplier,Category`, this.grid0.allowPaging)
  ...
  grid0LoadData(event: any) {
    this.northwind.getNorthwindProducts(`${event.filter ? event.filter + ' and ' : ''}ProductID eq ${this.parameters.ProductID}`, event.top, event.skip, `${event.orderby}`, `Supplier,Category`, event.top != null && event.skip != null)
    .subscribe((result: any) => {
      this.getNorthwindProductsResult = result.value;
