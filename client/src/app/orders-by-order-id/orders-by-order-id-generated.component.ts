...
export class OrdersByOrderIdGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  load() {
    this.northwind.getNorthwindOrders(`OrderID eq ${this.parameters.OrderID}`, this.grid0.allowPaging ? this.grid0.pageSize : null, this.grid0.allowPaging ? 0 : null, null, `Customer,Employee`, this.grid0.allowPaging, null, null)
  ...
  grid0LoadData(event: any) {
    this.northwind.getNorthwindOrders(`${event.filter ? event.filter + ' and ' : ''}OrderID eq ${this.parameters.OrderID}`, event.top, event.skip, `${event.orderby}`, `Customer,Employee`, event.top != null && event.skip != null, null, null)
    .subscribe((result: any) => {
      this.getNorthwindOrdersResult = result.value;
