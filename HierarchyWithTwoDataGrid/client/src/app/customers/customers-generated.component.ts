...
export class CustomersGenerated implements AfterViewInit, OnInit, OnDestroy {
  ...
  grid0RowExpand(event: any) {
    this.master = event;

    this.northwind.getOrders(`CustomerID eq '${event.CustomerID}'`, null, null, null, null, `Customer,Employee,Shipper`, null, null)
    .subscribe((result: any) => {
      event.Orders = result.value
    }, (result: any) => {

    });
  }

  grid1Add(event: any, data: any) {
    this.dialogService.open(AddOrderComponent, { parameters: {CustomerID: data.CustomerID}, title: 'Add Order' });
  }

  grid1Delete(event: any, data: any) {
  ...
  grid1RowSelect(event: any, data: any) {
